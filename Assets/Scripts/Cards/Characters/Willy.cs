...
public class Willy : MonoBehaviourPunCallbacks
{
    ...
    public void WillyFound(TrackableBehaviour mTrackableBehaviour)
    {
        bool condition = mTrackableBehaviour.TrackableName.ToString() == CustomEnums.Characters.WILLY_THE_KID.ToString() && mTrackableBehaviour.CurrentStatus.ToString() == "TRACKED" && mTrackableBehaviour.CurrentStatusInfo.ToString() == "NORMAL";
        if (condition) {
            ...
            if (PhotonNetwork.LocalPlayer.CustomProperties.ContainsKey("Character") && !PhotonNetwork.LocalPlayer.CustomProperties.ContainsKey("SpecialCharacter")) {
                if ((string)PhotonNetwork.LocalPlayer.CustomProperties["Character"] == CustomEnums.CharacterCards.SERIF.ToString()) {
                    print("[Willy] Player SERIF. Total number of lives = lives + 1");
                    livesNumber = livesNumber + 1;

                    _5lives.SetActive(true);
                }
                else {
                    print("[Willy] Player not SERIF.");
                ...
                customProperties.Add("SpecialCharacter", "Willy");
    ...
    public override void OnPlayerPropertiesUpdate(Player targetPlayer, PunHashtable changedProps)
    {
        ...
        if (targetPlayer != null) {
            if (changedProps.ContainsKey("SpecialCharacter")) {
                print("[Willy] SpecialCharacter custom property has changed! Player assigned to Willy.");
            }

            if (changedProps.ContainsKey("PlayerLives")) {
                print("[Willy] PlayerLives custom property has changed!");
